...
exports.preparerRedisPourGeohash = function (doc) {
    ...
    if (doc !== undefined && doc.rows !== undefined) {
        ...
        for (iCpt = 0; iCpt < length; iCpt = iCpt + 1) {
            if (doc.rows[iCpt].doc !== undefined) {
                coordsToAdd.push([doc.rows[iCpt].doc.geometry.coordinates[0], doc.rows[iCpt].doc.geometry.coordinates[1], doc.rows[iCpt].doc._id]);
            }
        }
    }
    return coordsToAdd;
...
exports.computeBoundsFromGeoHashAndCouch = function (redisDocsParsedByCouchId, response, db, proximityClient, roundLoc, latSW, lngSW, latNE, lngNE) {
    ...
    if (proximityClient !== undefined && lngSW !== undefined && latSW !== undefined && latNE !== undefined && lngNE !== undefined) {
        ...
        proximityClient.query(pointCalc.lng, pointCalc.lat, radiusTarget, {
            ...
            function generateCouchResponse() {
                ...
                len = places.length;
                for (iCpt = 0; iCpt < len; iCpt = iCpt + 1) {
                    docToReturn.rows.push({
                        doc: redisDocsParsedByCouchId[places[iCpt]]
                    });
                }
                return docToReturn;
            }
            ...
            //  console.log('lng, lat' + places.length);
            var docJson = generateCouchResponse();
            //console.log(docJson);

            var docToWorkOn = exports.preparerDocumentFeaturesFromCouchView(generateCouchResponse());
            //console.log(docToWorkOn);
            console.log('trying for ' + roundLoc + ' roundloc');
            var docToReturn = exports.retirerWaypointTropProche(docToWorkOn, roundLoc);
            console.log('done loading and rounding for ' + docToReturn.features.length + ' keys');

            response.json(docToReturn);
            //console.log(docToReturn);
        });
    }
}
