...
exports.generateGeoJsonDocBoundsDelta = function (documentSrc, latSW, lngSW, latNE, lngNE, dLatSW, dLngSW, dLatNE, dLngNE) {
    "use strict";
    var docReturn, pointCalc, len, radiusTarget, iCpt, polybounds, deltabounds;
    if (documentSrc !== undefined && documentSrc.name !== undefined && documentSrc.type !== undefined && lngSW !== undefined && latSW !== undefined && latNE !== undefined && lngNE !== undefined) {
        docReturn = {
            "name": documentSrc.name,
            "type": documentSrc.type,
        ...
        if (documentSrc.features !== undefined) {
            ...
            // Bounds des deux targets !
            polybounds = calculs.generatePolyFromBounds(latSW, lngSW, latNE, lngNE);
            deltabounds = calculs.generatePolyFromBounds(dLatSW, dLngSW, dLatNE, dLngNE);

            if (len >  0) { // Sanity check
                if (documentSrc.features[0].geometry.type === "Point") {
                    for (iCpt = 0; iCpt <  len; iCpt = iCpt + 1) {
                        if (calculs.isPointInPoly(deltabounds, documentSrc.features[iCpt].geometry.coordinates[0], documentSrc.features[iCpt].geometry.coordinates[1]) && calculs.isPointInPoly(polybounds, documentSrc.features[iCpt].geometry.coordinates[0], documentSrc.features[iCpt].geometry.coordinates[1]) === false) {
                            docReturn.features.push(documentSrc.features[iCpt]);
                        }
                    }
                }
