    ...
    if (documentSrc !== undefined && nbDecimalTolere !== undefined && documentSrc.features !== undefined) {
        ...
        for (iCpt = 0; iCpt < length; iCpt = iCpt + 1) {
            if (documentSrc.features[iCpt].geometry !== undefined && documentSrc.features[iCpt].geometry.coordinates !== undefined &&  typeof documentSrc.features[iCpt].geometry.coordinates[0] === 'number' && typeof documentSrc.features[iCpt].geometry.coordinates[1] === 'number') {
                latArrondis = documentSrc.features[iCpt].geometry.coordinates[0].toFixed(nbDecimalTolere);
                lngArrondis = documentSrc.features[iCpt].geometry.coordinates[1].toFixed(nbDecimalTolere);
...
exports.computeBoundsFromGeoHashAndCouch = function (redisDocsParsedByCouchId, response, db, proximityClient, roundLoc, latSW, lngSW, latNE, lngNE) {
    ...
    if (proximityClient !== undefined && lngSW !== undefined && latSW !== undefined && latNE !== undefined && lngNE !== undefined) {
        ...
        proximityClient.query(pointCalc.lng, pointCalc.lat, radiusTarget, {
            ...
            response.json(exports.retirerWaypointTropProche(generateCouchResponse(), roundLoc));
        });
    }
}
