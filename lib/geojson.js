...
exports.retirerWaypointTropProche = function (documentSrc, nbDecimalTolere) {
    var latArrondis, lngArrondis, featureToAdd, docReturn, length, iCpt, arrVerifDecimal;
    docReturn = {
        "name": documentSrc.name,
        "type": documentSrc.type,
        "features": []
    ...
    for (iCpt = 0; iCpt < length; iCpt = iCpt + 1) {
        latArrondis = parseFloat(featureToAdd.geometry.coordinates[0]).toFixed(nbDecimalTolere);
        lngArrondis = parseFloat(featureToAdd.geometry.coordinates[1]).toFixed(nbDecimalTolere);

        if (arrVerifDecimal[latArrondis,lngArrondis]) {
            continue; // Si on a deja la valeur arrondis ca sert a rien de reste la !
        } else
        {
            flags[latArrondis, lngArrondis] = true; // Garder la location en memoire !
            docReturn.features.push(documentSrc.features[iCpt]);
        }
    }
    return docReturn;
