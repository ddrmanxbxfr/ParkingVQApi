...
exports.preparerRedisPourGeohash = function (doc) {
    // Construire l'objet documentfeatures !
    "use strict";
    var coordsToAdd, iCpt, length;
    coordsToAdd = [];
    if (doc !== undefined && doc.rows !== undefined) {
        length = doc.rows.length; // Speedup js tricks :)
        for (iCpt = 0; iCpt < length; iCpt = iCpt + 1) {
            if (doc.rows[iCpt].doc !== undefined) {
                coordsToAdd.push([doc.rows[iCpt].doc.geometry.coordinates[0], doc.rows[iCpt].doc.geometry.coordinates[1], JSON.stringify(doc.rows[iCpt].doc)]);
            }
        }
    }
    return coordsToAdd;
...
exports.computeBoundsFromGeoHashAndCouch = function (response, db, proximityClient, roundLoc, latSW, lngSW, latNE, lngNE) {
